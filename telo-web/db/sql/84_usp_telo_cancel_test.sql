/*
 * 84_usp_telo_cancel_test.sql
 *
 * Cancels a SINGLE test line on a Telo bill — super-admin gated at the action
 * layer; this proc also refuses non-Telo bills. Behaviour (see the approved
 * plan):
 *   - Keeps the original bill line; adds a negative "(Cancelled)" offset line
 *     so the bill total drops and the pair is the on-bill audit trail.
 *   - Removes the cancelled test's ordered-test row (tbl_med_mcc_patient_tests).
 *   - If the test's sample (SID) is still REGISTERED (sample_status = 1), pulls
 *     just that test's code out of the SID's positionally-aligned
 *     testcodes/testnames/testtypes CSVs — keeping the SID (it may carry other
 *     tests, and is kept even if it becomes empty).
 *   - Onboarding is judged against the item's OWN sample codes — its code plus,
 *     for a profile, its constituent test codes (a split profile is stored on
 *     the SID as its individual codes). A test whose own sample type was never
 *     SID-tagged is removable even when the patient has SIDs for other tests.
 *   - Blocks if a carrying sample is already accessioned (sample_status > 1) or
 *     if the item is a master package (cancel those in the LIS).
 *   - Records who/when/why in dbo.telo_test_cancellation.
 *
 * Touches only Telo-managed rows the create-order SP already writes; no LIS
 * schema or LIS SP is involved. Returns { ok, error_code, message, balance }.
 */
...
BEGIN
    ...
    IF @carrying > 0 AND @accessioned > 0
    BEGIN
